...
"""
===========================================================================
AQRNN Ablation Study — AuverGrid (GWA-T-4) Dataset
===========================================================================
Identical pipeline to ablation_study.py, but configured for AuverGrid.
...
Three primary variants:
  ...
  C. Full MoE         — KMeans routing + per-cluster experts

IMPORTANT: Run process_auverGrid.py FIRST to generate the data files.

Usage:
  python ablation_study_auverGrid.py                  # Full run
  python ablation_study_auverGrid.py --quick          # Smoke test
  python ablation_study_auverGrid.py --variants-only  # Only A/B/C
===========================================================================
"""
...
# Output (AuverGrid-specific names)
RESULTS_JSON   = os.path.join(PROJECT_ROOT, "ablation_results_auverGrid.json")
PLOT_DIR       = os.path.join(PROJECT_ROOT, "ablation_plots_auverGrid")
...
def load_shared_data(n_components=N_COMPONENTS, n_clusters=N_CLUSTERS):
    ...
    # Sort by time for strict temporal split
...
def print_variant_table(results):
    ...
    print("\n")
    print("╔" + "═" * 95 + "╗")
    print("║" + "  ABLATION STUDY — PRIMARY VARIANT COMPARISON (AuverGrid)".center(95) + "║")
    print("╚" + "═" * 95 + "╝")
...
def print_grid_table(grid_results):
    ...
    print("\n")
    print("╔" + "═" * 110 + "╗")
    print("║" + "  ABLATION GRID RESULTS (AuverGrid)".center(110) + "║")
    print("╚" + "═" * 110 + "╝")
...
def generate_plots(variant_results, grid_results=None):
    ...
    if valid:
        ...
        plt.suptitle("Ablation Study: Primary Variant Comparison (AuverGrid)",
    ...
    for vname, vm in variant_results.items():
        if vm and "per_cluster" in vm:
            ...
            plt.suptitle(f"Per-Cluster Metrics — {vname} (AuverGrid)",
    ...
    if grid_results:
        ...
        if valid_grid:
            ...
            ax.set_title("Ablation Grid — RMSE Ranking (AuverGrid, lower is better)",
...
# ══════════════════════════════════════════════════════════════
def main():
    parser = argparse.ArgumentParser(description="AQRNN Ablation Study (AuverGrid)")
    ...
    print("╔══════════════════════════════════════════════════════════════╗")
    print("║    AQRNN ABLATION STUDY — AuverGrid (GWA-T-4) Dataset     ║")
    print("╠══════════════════════════════════════════════════════════════╣")
    ...
    print(f"║  CSV: auverGrid_hybrid_clean.csv                           ║")
    print(f"║  Fed: federated_data_auverGrid/                            ║")
    print("╚══════════════════════════════════════════════════════════════╝\n")
    ...
    print("\n✅ Ablation study (AuverGrid) complete!")
