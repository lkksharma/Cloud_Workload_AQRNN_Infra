...
"""
══════════════════════════════════════════════════════════════════════
Post-Quantum Ridge Correction — AuverGrid (GWA-T-4) Dataset
══════════════════════════════════════════════════════════════════════

Loads trained CFL experts from cfl_experts_updated_auverGrid.pkl,
identifies the worst-performing expert, trains a Ridge regression
on its residuals, and reports corrected overall metrics.

IMPORTANT: Run cfl_full_moe_auverGrid.py FIRST to generate the
expert checkpoint.

Usage:
    python post_quantum_ridge_auverGrid.py
══════════════════════════════════════════════════════════════════════
"""
...
# Import AQRNN components (from project root)
...
# ══════════════════════════════════════════════════════════════
# CONFIG — AuverGrid paths
# ══════════════════════════════════════════════════════════════
CSV_PATH      = os.path.join(PROJECT_ROOT, "auverGrid_hybrid_clean.csv")
KMEANS_PATH   = os.path.join(PROJECT_ROOT, "kmeans_model_auverGrid.pkl")
FED_RESULTS   = os.path.join(PROJECT_ROOT, "cfl_experts_updated_auverGrid.pkl")
SEQ_LEN       = 2
N_H           = 4
HIDDEN_DIM    = 64
N_LAYERS      = 1
...
DEVICE_MODE   = "cuda"  # Must match CFL training
...
def load_test_data():
    """Load global test set (same pipeline as CFL)."""
    ...
    # Try to load pre-saved KMeans model; if not found, refit from data
    if os.path.exists(KMEANS_PATH):
        ...
        print(f"  ⚠ {KMEANS_PATH} not found — refitting from data")
        scaler_x = QuantileTransformer(output_distribution='uniform',
                                       n_quantiles=min(1000, len(df)))
        X_raw_all = df[features].values
        X_scaled_all = scaler_x.fit_transform(X_raw_all)
        pca_obj = PCA(n_components=4, random_state=42)
        X_pca_all = pca_obj.fit_transform(X_scaled_all)
        kmeans = KMeans(n_clusters=3, random_state=42, n_init=20, init='k-means++')
        kmeans.fit(X_pca_all)
        # Save for future use
        with open(KMEANS_PATH, "wb") as f:
            pickle.dump({"scaler_x": scaler_x, "pca": pca_obj, "kmeans": kmeans}, f)
...
if __name__ == "__main__":
    print("╔══════════════════════════════════════════════════════════╗")
    print("║  Post-Quantum Ridge Correction — AuverGrid (GWA-T-4)   ║")
    print("╚══════════════════════════════════════════════════════════╝\n")

    # Load trained experts
    if not os.path.exists(FED_RESULTS):
        print(f"ERROR: {FED_RESULTS} not found!")
        print("Run cfl_full_moe_auverGrid.py first to generate expert checkpoint.")
        sys.exit(1)
    ...
    # Only correct the worst-performing expert
    ...
    print("\n" + "=" * 80)
    print("  RESULTS: Before vs After Residual Correction (AuverGrid)")
    print("=" * 80)
    ...
    results = {
        ...
        "ridge_params_per_cluster": SEQ_LEN * n_x + 2,
    }
    results_path = os.path.join(PROJECT_ROOT, "residual_correction_results_auverGrid.json")
    with open(results_path, "w") as f:
        json.dump(results, f, indent=2, default=str)

    # ── FINAL SUMMARY ──
    ridge_params = (SEQ_LEN * n_x + 1 + 1) * data["n_clusters"]
    print("\n" + "═" * 80)
    print("  COMPLETE ARCHITECTURE: CFL AQRNN + Post-Quantum Ridge (AuverGrid)")
    print("═" * 80)
    print(f"""
  ┌─────────────────────────────────────────────────────────────────┐
  │  PIPELINE                                                       │
  ...
  │                                                                 │
  │  Training: CFL (FedAvg per expert) + Ridge on residuals        │
  └─────────────────────────────────────────────────────────────────┘
...
""")
    ...
    print("  ╔══════════════════════════════════════════════════════════════╗")
    print("  ║  FINAL RESULTS: CFL AQRNN + Post-Quantum Ridge (AuverGrid) ║")
    print("  ╠══════════════════════════════════════════════════════════════╣")
